...
import { useEffect, useState } from 'react';
import { useParams, Link } from 'react-router-dom';
// We no longer need the Copy icon
// import { Copy } from 'lucide-react';
...
export default function SessionPage() {
  ...
  const [session, setSession] = useState(null);
  const [loading, setLoading] = useState(true);
  // The 'rewriting' state and 'handleRewrite' function have been removed.

  useEffect(() => {
    const fetchSession = async () => {
      try {
        const apiUrl = `${import.meta.env.VITE_API_BASE_URL}/api/session/${id}`;
  ...
  const matchingKeywords = session.keywords.resumeKeywords.filter(k => session.keywords.jdKeywords.includes(k));
  const missingKeywords = session.keywords.jdKeywords.filter(k => !session.keywords.jdKeywords.includes(k));
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-8">
        ...
        {/* The "Rewrite Suggestions" section has been completely removed. */}
    </div>
  );
}

// The ScoreCard and KeywordCard components remain the same.
const ScoreCard = ({ title, score }) => (
    <div className="bg-white p-6 rounded-lg shadow text-center">
        <h3 className="text-lg font-medium text-gray-500">{title}</h3>
        <p className={`text-5xl font-bold ${score > 75 ? 'text-green-500' : score > 50 ? 'text-yellow-500' : 'text-red-500'}`}>{score}<span className="text-2xl text-gray-400">/100</span></p>
